...
void showMenu(vector<MenuItem> &m)
{
  ...
  cout << "H) \t Add a tip(Preferebly 20%) and pay\t" << endl;
}

void printReceipt(vector<MenuItem> &m, double subtotal, double cashBack)
{
  cout << "Thank you for choosing Soup" << endl;
  cout << "We are not responsible for refunds and have the right to refuse service" << endl;
  cout << "Failure to comply will put you in jail" << endl;
  cout << "Your order number is " << rand() % 10 + 1 << endl;

  cout << "Total: " << subtotal << endl;
  cout << "Change: " << cashBack << endl;
    ...
    for(int i=0; i < m.size(); i++)
    {
      ...
      else if(option == 'h' || option == 'H')
      {
        ...
        else if(y == "cc" || y == "CC")
        {
          ...
          cout << "Accept?(Failure to accept will result in jail, A to accept, D to decline): ";
          char p;
          cin >> p;
          if(p == 'd' || p == 'D')
          {
